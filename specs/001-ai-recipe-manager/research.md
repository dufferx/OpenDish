...
## R14. Social media video recipe import (`yt-dlp`) — approved metadata-first scope

**Status**: Approved for implementation on 2026-08-24. Phase 11.75 includes Instagram Reels, TikTok videos, and YouTube Shorts. The approved first increment is metadata/description extraction only; audio transcription is deferred to a future phase.
...
**Decision**: follow the `yt-dlp`-based approach for all three platforms:
...
2. When the caption alone has no usable recipe, Phase 11.75 reports a clear recoverable failure and offers the existing paste-text fallback. A future phase may add `yt-dlp -x --audio-format mp3` plus OpenAI transcription; that work is deliberately not part of the current implementation.
...
- **Instagram**: public Reels may be accessible to `yt-dlp` without cookies depending on the requesting context, IP, URL, and current platform behavior. The user's observed public-Reel flow is therefore the baseline for this phase. Cookies are not a prerequisite or an approved dependency; if a particular public Reel cannot be extracted, the user receives the normal recoverable failure/fallback. Extraction remains subject to Instagram changes and intermittent upstream blocking.
...
**Approved shape**: a small, version-pinned HTTP microservice (Node or Python, `yt-dlp` installed; `ffmpeg` reserved for future transcription) on a container host (Fly.io/Render/Cloud Run — free/near-free tiers are acceptable within the project's existing cost constraint). `import-recipe` calls it over HTTPS with a shared secret for Instagram, TikTok, and YouTube Shorts, gets back `{ title, description }`, and feeds that into the existing `extractRecipe` pipeline unchanged. No `RecipeDraft` schema change is needed — only a new `extractionMethod: 'video_metadata'` alongside the existing `'structured_markup'` / `'ai'`.
...
- **Third-party paid scraping APIs** (RapidAPI-style Instagram/TikTok scrapers): recurring per-call cost and another vendor dependency. Not selected for Phase 11.75; the approved baseline is the project's own `yt-dlp` microservice, while a paid provider remains a possible operational fallback if upstream blocking makes the service unusable.
- **Keep only the existing "Paste text" fallback**: zero engineering/hosting/ToS cost, already shipped today. Remains the recoverable fallback when metadata extraction does not produce a usable recipe.

**Known risks/costs retained for implementation and operations**:
1. **Instagram behavior can vary by request context.** The phase therefore deliberately avoids making session cookies a requirement. A leaked session cookie would be a materially bigger trust/security surface than the existing OpenAI API key BYOK model, so cookies are excluded unless a later phase separately approves them.
2. **A new hosting component extends the current "zero/near-zero hosting cost" constraint** (plan.md) and the "Supabase + static frontend, nothing else" architecture — it must remain small, authenticated, rate-limited, and deployed within the approved cost-conscious profile.
...
1. Instagram Reels, TikTok videos, and YouTube Shorts are all approved for Phase 11.75 with no intentional platform limitation.
2. The implementation must not require Instagram session cookies. Cookie custody is explicitly out of scope.
3. A separately hosted `yt-dlp` microservice is approved as the necessary architecture extension, while preserving the existing zero/near-zero-cost hosting preference.
4. Audio transcription is deferred to a future phase and is not attempted by the Phase 11.75 implementation.
